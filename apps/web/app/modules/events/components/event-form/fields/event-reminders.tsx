...
interface EventReminderTimesProps {
  reminderTimes?: ReminderTime[];
  onReminderTimesChange: (times: ReminderTime[]) => void;
}

interface EventReminderEmailCustomizationProps {
  customMessage?: string | null;
...
export function EventReminderTimes({
  reminderTimes = [],
  onReminderTimesChange,
}: EventReminderTimesProps) {
  ...
  return (
    <section className="space-y-4">
      <p className="text-sm text-muted-foreground">
        Send reminder emails before the event. Select any number of times.
      </p>
      ...
      {selectedTimes.length > 0 && (
        <div className="bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-lg p-3 text-sm">
          <p className="text-blue-900 dark:text-blue-100">
            ✓ Reminders will be sent {selectedTimes.length} time
            {selectedTimes.length !== 1 ? "s" : ""} per attendee
          </p>
        </div>
      )}
    </section>
...
export function EventReminderEmailCustomization({
  customMessage = null,
  onCustomMessageChange,
}: EventReminderEmailCustomizationProps) {
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        Personalize the confirmation and reminder emails attendees receive.
      </p>
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          Leave unchecked to use the default message, or check to
      ...
      <div className="bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-800 rounded-lg p-3 text-sm">
        <p className="text-amber-900 dark:text-amber-100">
          💡 Tip: Keep messages clear and include all relevant event
          details. Variables will be automatically replaced with actual
          values.
        </p>
      </div>
    </div>
