...
import type { Route } from './+types/api.google-forms.$formId.responses';
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  try {
    ...
    const responses = await getFormResponses(oauth2Client, formId);
    const parsedResponses = parseResponses(responses, questions);
    ...
    return Response.json(
      { error: 'Failed to get responses', message: error.message },
      { status: 500, headers }
    );
  }
}
