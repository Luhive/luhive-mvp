...
import { getEmailConfig, sendVerificationEmail } from "~/shared/lib/email.server";
...
export async function loader({ request }: LoaderFunctionArgs) {
	...
	if (sendTest && testEmail && isDevelopment) {
		try {
			...
			if (!emailRegex.test(testEmail)) {
				testResult = { success: false, error: `Invalid test email format: ${testEmail}` };
			} else {
		...
		testResult = { success: false, error: "Test email sending is only allowed in development mode" };
	} else if (sendTest && !testEmail) {
		testResult = { success: false, error: "testEmail parameter is required when sendTest=true" };
	}
	...
	return {
		...
		testResult,
		instructions: {
			usage: "Visit /api/events/email-debug to see configuration status",
			testEmail: "Add ?testEmail=your@email.com&sendTest=true to send a test email (development only)",
		},
	};
}
