...
export function LandingNavbar() {
  ...
  return (
    <nav
      className={`fixed inset-x-0 top-0 z-50 transition-colors ${scrolled ? 'bg-[#F6F4F1]/95 backdrop-blur-md' : 'bg-[#F6F4F1]'
        }`}
    >
          ...
          {navItems.map((item) => {
            ...
            return (
              <a
                key={item.id}
                href={item.href}
                className={`relative flex px-[0.625rem] py-[0.125rem] justify-center items-center gap-2 self-stretch transition-colors ${isActive ? 'text-foreground' : 'hover:text-foreground'
                  }`}
              >
      ...
      {/* Backdrop */}
      <div
        className={`fixed inset-0 z-60 bg-black/50 transition-opacity duration-300 ease-out md:hidden ${isMobileMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'
          }`}
        onClick={() => setIsMobileMenuOpen(false)}
      />
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 right-0 z-70 w-full bg-[#F6F4F1] h-[100svh] shadow-xl md:hidden transition-transform duration-300 ease-out ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
      >
        <div className="flex w-[80vw] mx-auto h-full flex-col bg-[#F6F4F1]">
            ...
            {navItems.map((item, index) => {
              ...
              return (
                <a
                  ...
                  href={item.href}
                  onClick={() => setIsMobileMenuOpen(false)}
                  className={`text-2xl font-medium transition-all duration-300 ${isMobileMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
          ...
          <div
            className={`p-6 transition-all duration-300 ${isMobileMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
              }`}
            style={{
